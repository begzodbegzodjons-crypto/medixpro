...
export default function Stats() {
  ...
  return (
    <div>
      <div className="mb-6"><h2 className="text-xl font-medium text-neutral-900 mb-1">Statistika</h2><p className="text-sm text-neutral-500">O'qish jarayoningiz</p></div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {cards.map((card, i) => (
          <div key={i} className="border border-neutral-200 rounded-lg p-6">
            <p className="text-sm text-neutral-500 mb-2">{card.title}</p>
        ...
        <div className="border border-neutral-200 rounded-lg overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-neutral-50"><tr><th className="px-4 py-3 text-left text-sm font-medium text-neutral-500">Test</th><th className="px-4 py-3 text-left text-sm font-medium text-neutral-500 hidden md:table-cell">Fan</th><th className="px-4 py-3 text-left text-sm font-medium text-neutral-500">Ball</th><th className="px-4 py-3 text-left text-sm font-medium text-neutral-500">Natija</th></tr></thead>
              <tbody className="divide-y divide-neutral-100">
                {stats.recentResults.map((r: any) => (
                  <tr key={r.id} className="hover:bg-neutral-50">
                    ...
                    <td className="px-4 py-3"><span className={`px-2.5 py-1 rounded text-xs font-medium ${r.passed ? 'bg-neutral-900 text-white' : 'bg-neutral-100 text-neutral-600'}`}>{r.passed ? "O'tdi" : 'Yutqazdi'}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
