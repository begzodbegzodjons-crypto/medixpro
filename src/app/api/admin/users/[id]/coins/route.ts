...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await transaction(async (conn) => {
      ...
      const tId = generateId(); await conn.execute("INSERT INTO Transaction (id, userId, type, amount, description, balanceBefore, balanceAfter, createdAt) VALUES (?, ?, 'admin_adjustment', ?, ?, ?, ?, NOW())", [tId, id, balanceAfter - balanceBefore, 'Admin tomonidan balans tahrirlandi', balanceBefore, balanceAfter])
    })
