...
export async function POST(request: Request) {
  try {
    ...
    const result = await transaction(async (tx) => {
      const matRes: any = await tx.execute('SELECT id, title, price, isFree FROM Material WHERE id = ?', [materialId])
      ...
      const existRes: any = await tx.execute('SELECT id FROM Purchase WHERE userId = ? AND materialId = ?', [user.id, materialId])
      const existing = existRes.rows || existRes
      if (existing && existing.length > 0) throw new Error('Siz allaqachun sotib olgansiz')
      if (Boolean(material.isFree) || price === 0) {
        const pId = generateId()
        await tx.execute('INSERT INTO Purchase (id, userId, materialId, price, createdAt) VALUES (?, ?, ?, 0, NOW())', [pId, user.id, materialId])
        const lId = generateId()
        await tx.execute('INSERT INTO Library (id, userId, materialId, createdAt) VALUES (?, ?, ?, NOW())', [lId, user.id, materialId])
        return { success: true, price: 0, title: material.title }
      }
      const userRes: any = await tx.execute('SELECT coinBalance FROM User WHERE id = ?', [user.id])
      ...
      const pId = generateId()
      await tx.execute('INSERT INTO Purchase (id, userId, materialId, price, createdAt) VALUES (?, ?, ?, ?, NOW())', [pId, user.id, materialId, price])
      const lId = generateId()
      await tx.execute('INSERT INTO Library (id, userId, materialId, createdAt) VALUES (?, ?, ?, NOW())', [lId, user.id, materialId])
      const tId = generateId()
      await tx.execute("INSERT INTO Transaction (id, userId, type, amount, description, balanceBefore, balanceAfter, createdAt) VALUES (?, ?, 'purchase', ?, ?, ?, ?, NOW())", [tId, user.id, -price, `Sotib olindi: ${material.title}`, currentBalance, balanceAfter])
