...
export async function POST(request: Request) {
  try {
    ...
    const result = await transaction(async (conn) => {
      const [rows]: any = await conn.query('SELECT id, name, coins, code, isActive FROM CoinPackage WHERE code = ?', [String(code).trim().toUpperCase()])
      ...
      const tId = generateId(); await conn.execute("INSERT INTO Transaction (id, userId, type, amount, description, balanceBefore, balanceAfter, createdAt) VALUES (?, ?, 'coin_package', ?, ?, ?, ?, NOW())", [tId, user.id, Number(pkg.coins), `COIN paketi: ${pkg.name} (${pkg.code})`, balanceBefore, balanceAfter])
