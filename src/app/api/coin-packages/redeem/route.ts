...
export async function POST(request: Request) {
  try {
    ...
    if (!code) return NextResponse.json({ message: 'Kod talab qilinadi' }, { status: 400 })

    const result = await transaction(async (tx) => {
      const pkgRes: any = await tx.execute('SELECT id, name, coins, code, isActive FROM CoinPackage WHERE code = ?', [String(code).trim().toUpperCase()])
      ...
      const balanceBefore = Number(users[0].coinBalance); const balanceAfter = balanceBefore + Number(pkg.coins)
      await tx.execute('UPDATE User SET coinBalance = ? WHERE id = ?', [balanceAfter, user.id])
      const tId = generateId()
      await tx.execute("INSERT INTO Transaction (id, userId, type, amount, description, balanceBefore, balanceAfter, createdAt) VALUES (?, ?, 'coin_package', ?, ?, ?, ?, NOW())", [tId, user.id, Number(pkg.coins), `COIN paketi: ${pkg.name} (${pkg.code})`, balanceBefore, balanceAfter])
