...
export async function POST(request: Request) {
  try {
    ...
    const result = await transaction(async (tx) => {
      const acRes: any = await tx.execute('SELECT id, code, type, isUsed FROM AdminCode WHERE code = ?', [String(code).trim().toUpperCase()])
      ...
      await tx.execute('UPDATE AdminCode SET isUsed = 1, usedById = ?, usedAt = NOW() WHERE id = ?', [user.id, adminCode.id])
      if (adminCode.type === 'admin') await tx.execute('UPDATE User SET isAdmin = 1 WHERE id = ?', [user.id])
