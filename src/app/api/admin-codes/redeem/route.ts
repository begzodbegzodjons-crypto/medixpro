...
export async function POST(request: Request) {
  try {
    ...
    const result = await transaction(async (conn) => {
      const [rows]: any = await conn.query('SELECT id, code, type, isUsed FROM AdminCode WHERE code = ?', [String(code).trim().toUpperCase()])
      if (rows.length === 0) throw new Error('Noto\'g\'ri kod')
      const adminCode = rows[0]
      if (Boolean(adminCode.isUsed)) throw new Error('Bu kod allaqachun ishlatilgan')
      await conn.execute('UPDATE AdminCode SET isUsed = 1, usedById = ?, usedAt = NOW() WHERE id = ?', [user.id, adminCode.id])
      if (adminCode.type === 'admin') await conn.execute('UPDATE User SET isAdmin = 1 WHERE id = ?', [user.id])
